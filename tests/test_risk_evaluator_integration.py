...
from arbitrage_test_data_new import new_arbitrage_test_data
...
def test_integrated_detector_with_risk():
    ...
    # Build graph from new test data
    builder = TokenGraphBuilder()
    graph = builder.build_graph_from_edge_lists(new_arbitrage_test_data)
    print(
    ...
    print(f"\n--- Comparison ---")
    print(
        f"Risk evaluation filtered out: {len(opportunities_without_risk) - len(opportunities_with_risk)} opportunities")
